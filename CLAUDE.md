...
- **Single-record detail/drilldown pages** — any screen showing one saved record in a read-focused, non-tabular, non-editing layout (e.g. viewing one Segment, Account, or Engagement) MUST reuse `PxDetailDrilldownShell` from `src/patterns/px-detail-drilldown-shell`. It composes `PxMainContainer` directly (another sanctioned direct-consumer context) and reuses `PxHeader`'s existing Secondary Bar props for the back-arrow + title + optional tabs + optional utilities/actions anatomy — do NOT hand-roll a second header or back-navigation control. Never name or structure the shell around a specific entity; the entity name only ever appears in the `title` prop's runtime value.
